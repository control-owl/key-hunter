...
__device__ void scalarMultiplication(unsigned int pubX[8], unsigned int pubY[8], const uint8_t scalar[32]) {
    ...
    // Shared memory load for precomputes
    // 64 points for w=6
    // 128 points for w=7
    __shared__ unsigned int shared_precomp_x[64][8];
    __shared__ unsigned int shared_precomp_y[64][8];

    if (threadIdx.x < 64) {
        #pragma unroll
        for (int j = 0; j < 8; ++j) {
            shared_precomp_x[threadIdx.x][j] = PRECOMP_X[threadIdx.x][j];
            shared_precomp_y[threadIdx.x][j] = PRECOMP_Y[threadIdx.x][j];
        }
    }
    __syncthreads();

    int processed_bits = 0;
    const int PRECOMP_BITS = 72;

    while (processed_bits < PRECOMP_BITS) {
        int bit_pos_local = processed_bits;
        int byte_idx = bit_pos_local / 8;
        int bit_in_byte = bit_pos_local % 8;

        // Correct 7-bit nibble (mask 0x7F)
        // Correct 6-bit nibble (mask 0x3F)
        unsigned int nibble = (scalar[byte_idx] >> bit_in_byte) & 0x3F;

        if (bit_in_byte + W > 8) {
            int bits_needed = bit_in_byte + W - 8;
            nibble |= ((scalar[byte_idx + 1] & ((1u << bits_needed) - 1)) << (8 - bit_in_byte));
        }

        // Always advance by 'W' bits (even if zero)
        processed_bits += W;

        // if (nibble == 0) {
        //     continue;  // Zero window: no add, doubles done later in main loop
        // }

        // Non-zero: add precomputed point (nibble 1-64)
        if (nibble < 64) {
            ...
            jacobianMixedAdd(
                tempX, tempY, tempZ, 
                X, Y, Z, 
                shared_precomp_x[nibble], shared_precomp_y[nibble]
            );
    ...
    unsigned int denominator[8], inv_z[8], temp[8];
